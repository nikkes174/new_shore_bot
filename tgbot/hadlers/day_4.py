...
from config import AUDIO_ID_DAY_4, DOCUMENT_ID_4
from tgbot.keyboars.user_replay_keyboards import (
    day4_start_keyboard,
    day4_video_keyboard,
    day4_done_keyboard, day4_after_podcast_keyboard,
)
...
async def day4_podcast(callback: types.CallbackQuery):
    ...
    await callback.message.answer_audio(
        AUDIO_ID_DAY_4,
        reply_markup=day4_after_podcast_keyboard()
    )
...
async def day4_video(callback: types.CallbackQuery):
    ...
    await callback.message.answer_document(
        DOCUMENT_ID_4, reply_markup=day4_done_keyboard()
    )
