...
async def send_day6(bot, user_id: int):

    with open("files/6/1.txt", encoding="utf-8") as f:
        text = f.read()

    await bot.send_message(user_id, text, reply_markup=day6_start_keyboard())


@router.message(F.text == "/day6")
async def start_day6(message: types.Message):
    await send_day6(message.bot, message.from_user.id)

    with open("files/6/1.txt", encoding="utf-8") as f:
        text = f.read()

    await message.answer(text, reply_markup=day6_start_keyboard())
...
async def day6_podcast(callback: types.CallbackQuery):
    ...
    await callback.message.answer_audio(
        AUDIO_ID_DAY_6, reply_markup=day6_listened_keyboard()
    )
...
async def day6_task(callback: types.CallbackQuery):
    ...
    await callback.message.answer(text, reply_markup=day6_done_keyboard())
...
async def day6_done(callback: types.CallbackQuery):
    ...
    await callback.message.answer("Отлично. Впереди заключительный день.\nДо завтра.")
